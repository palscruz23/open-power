...
- Protection-ready palette entries should stay within the existing asset taxonomy by dropping a normal asset node plus prefilled `data.protection` defaults; do not add standalone protection node types unless the frontend/editor and backend `asset_type` contract both change together.
...
- Arc-flash canvas overlays should stay in transient node fields only; if you add frontend result annotations, update both `TRANSIENT_NODE_DATA_KEYS`/`sanitizeGraphForPersistence()` and the study reset paths in `LoadFlowStudyPage.jsx` so saved diagrams do not retain arc-flash output.
- Arc-flash safety labels should come from the backend `guidance` block rather than being inferred from raw summary values in the frontend; when extending review output, keep PPE/severity availability, cautions, and unsupported-label reasons aligned between `backend/main.py`, `ControlPanel.jsx`, and the transient canvas annotation mapping in `LoadFlowStudyPage.jsx`.
