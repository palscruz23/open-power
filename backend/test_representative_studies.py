...
class RepresentativeStudyVerificationTests(unittest.TestCase):
    def test_load_flow_sample_network_returns_bus_line_and_load_results(self):
        ...
        self.assertEqual(result['lines']['line-1']['from_bus_id'], 'bus-1')
        self.assertEqual(result['lines']['line-1']['to_bus_id'], 'bus-2')
    ...
    def test_ansi_short_circuit_sample_network_returns_fault_and_branch_contributions(self):
        ...
        self.assertEqual(result['branches']['line-1']['result_key'], 'ikss_ka')
        self.assertEqual(result['branches']['line-1']['result_label'], 'ANSI symmetrical RMS current')
        self.assertIn('limitations', result['fault'])
    ...
    def test_iec_60909_short_circuit_sample_network_returns_thermal_current_metadata(self):
        ...
        self.assertEqual(result['branches']['line-1']['result_label'], 'Thermal equivalent current')
        self.assertIsNotNone(result['branches']['line-1']['ith_ka'])
        self.assertIsNone(result['branches']['line-1']['from_ikss_ka'])
    ...
    def test_protection_coordination_sample_network_returns_curves_and_analysis(self):
        ...
        self.assertTrue(all(curve['max_fault_current_a'] > curve['load_current_a'] for curve in result['curves']))
        self.assertEqual([curve['device_type'] for curve in result['curves']], ['oc_relay', 'fuse'])
